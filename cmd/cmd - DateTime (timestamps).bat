...
REM ------------------------------------------------------------
REM
REM CMD - Get Timestamp (the robust way, based on w32tm)
REM 
...
REM SET TIME_SINCE_LAST_SYNC=6.8288064s
SET TIME_SINCE_LAST_SYNC=157680000.440s
FOR /f "tokens=1 delims=/." %a IN ('ECHO %TIME_SINCE_LAST_SYNC:s=%') DO SET /A SECONDS_SINCE_LAST_SYNC=%a
FOR /f "tokens=2 delims=/." %a IN ('ECHO %TIME_SINCE_LAST_SYNC:s=%') DO SET /A DECIMAL_SEC_SINCE_LAST_SYNC=%a
SET /A REMAINDER_SECONDS=SECONDS_SINCE_LAST_SYNC
SET /A YEARS_SINCE_LAST_SYNC=(((REMAINDER_SECONDS/31536000)%31536000+31536000)%31536000)
SET /A REMAINDER_SECONDS=REMAINDER_SECONDS-(YEARS_SINCE_LAST_SYNC*31536000)
SET /A MONTHS_SINCE_LAST_SYNC=(((REMAINDER_SECONDS/2628000)%2628000+2628000)%2628000)
SET /A REMAINDER_SECONDS=REMAINDER_SECONDS-(MONTHS_SINCE_LAST_SYNC*2628000)
SET /A DAYS_SINCE_LAST_SYNC=(((REMAINDER_SECONDS/86400)%86400+86400)%86400)
SET /A REMAINDER_SECONDS=REMAINDER_SECONDS-(DAYS_SINCE_LAST_SYNC*86400)
SET /A HRS_SINCE_LAST_SYNC=(((REMAINDER_SECONDS/3600)%3600+3600)%3600)
SET /A REMAINDER_SECONDS=REMAINDER_SECONDS-(HRS_SINCE_LAST_SYNC*3600)
SET /A MIN_SINCE_LAST_SYNC=(((REMAINDER_SECONDS/60)%60+60)%60)
SET /A REMAINDER_SECONDS=REMAINDER_SECONDS-(MIN_SINCE_LAST_SYNC*60)
