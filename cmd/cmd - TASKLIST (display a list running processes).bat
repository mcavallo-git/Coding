...
FOR /f "tokens=1 delims=/." %a IN ('ECHO %TIME_SINCE_LAST_SYNC:s=%') DO SET /A SEC_SINCE_LAST_SYNC=%a
FOR /f "tokens=2 delims=/." %a IN ('ECHO %TIME_SINCE_LAST_SYNC:s=%') DO SET /A DECIMAL_SEC_SINCE_LAST_SYNC=%a
SET /A REMAINDER_SECONDS=SEC_SINCE_LAST_SYNC
SET /A DAYS_TO_ADD=(((REMAINING_SECONDS/86400)%86400+86400)%86400)
SET /A REMAINDER_SECONDS=REMAINDER_SECONDS-(DAYS_TO_ADD*86400)
...
SET /A MIN_SINCE_LAST_SYNC=(((SEC_SINCE_LAST_SYNC/60)%60+60)%60)
SET /A REMAINDER_SECONDS=REMAINDER_SECONDS-(MIN_SINCE_LAST_SYNC*60)
SET /A SEC_TO_ADD=REMAINDER_SECONDS
...
SET /A DAYS_TO_ADD=(DAYS_TO_ADD+((((LAST_SYNC_HR+HRS_TO_ADD)/24)%24+24)%24))
