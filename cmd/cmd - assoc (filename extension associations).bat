REM ------------------------------------------------------------
REM
REM   cmd - assoc (filename extension associations)
REM
REM ------------------------------------------------------------

REM List file name extension associations active for current session
assoc


REM Treat extensionless files as though they were text files
assoc .=txtfile


REM General Syntax
assoc [.EXTENSION]=[NEW_ASSOCIATION]


REM ------------------------------------------------------------
REM
REM Citation(s)
REM
REM   docs.microsoft.com  |  "Displays or modifies file name extension associations"  |  https://docs.microsoft.com/en-us/windows-server/administration/windows-commands/assoc
REM
REM ------------------------------------------------------------