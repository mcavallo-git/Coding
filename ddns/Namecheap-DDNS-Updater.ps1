...
$nc_subdomain = Get-Content -Path (($Home)+("/.namecheap/subdomain"));
$nc_domain = Get-Content -Path (($Home)+("/.namecheap/domain"));
$nc_token = Get-Content -Path (($Home)+("/.namecheap/token"));
$nc_ip = "";

$nc_urlPlaintext = (("https://dynamicdns.park-your-domain.com/update?host=")+($nc_subdomain)+("&domain=")+($nc_domain)+("&password=")+($nc_token)+("&ip=")+($nc_ip));
