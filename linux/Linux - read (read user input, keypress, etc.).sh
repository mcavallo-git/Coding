...
read -p "Press any key to continue...  " -n 1 -t 60 REPLY <'/dev/tty'; # Await single keypress (works with curl'ed scripts)

read -p "Press any key to continue...  " -n 1 -t 60 -r; # Await single keypress (using [ -r ] breaks on curl commands (fails read immediately))
...
if [ 1 -eq 1 ]; then
	READ_TIMEOUT=60;
	read -p "Enter a string:  " -t ${READ_TIMEOUT} REPLY <'/dev/tty'; RETURN_CODE_READ=$?;
	echo "";
...
if [ 1 -eq 1 ]; then
  ...
  READ_TIMEOUT=3;
  read -p "Perform action [ ${ACTION_DESCRIPTION} ], now? (y/n)  " -n 1 -t ${READ_TIMEOUT} REPLY <'/dev/tty'; RETURN_CODE_READ=$?;
  echo "";
...
if [ 1 -eq 1 ]; then
  ...
  echo -e "      |";
  read -p "      |--> Are you sure you want to continue? (y/n)  " -n 1 -t 60 REPLY <'/dev/tty'; # Await single keypress
  echo -e "";
  if [ -n "${REPLY}" ] && [ "$(echo ${REPLY} | tr '[:lower:]' '[:upper:]';)" == "Y" ]; then
    echo -e "      |";
    read -p "      |--> Are you completely positive you wish to continue? (y/n)  " -n 1 -t 60 REPLY <'/dev/tty'; # Await single keypress
    echo -e "";
    if [ -n "${REPLY}" ] && [ "$(echo ${REPLY} | tr '[:lower:]' '[:upper:]';)" == "Y" ]; then
