#
#	Note: Phrases such as "this Monday" become muddled & hard to identify when "today" is a Saturday/Sunday (weekend day).
#
...
#	  
#	  	The weekend prevents simple arithmetic such as "2 days is less than 5 so 'this Monday' must refer to the next Monday" from being correct, as Saturday appears so much ((visually) closer to the previous week on a paper calendar (lending to "this" referring to "previous", keeping it in the picture).
#	  
...
$LastMondaysDate = (Get-Date 0:00).AddDays(1 + ([Int](Get-Date).DayOfWeek * -1));
