...
| ForEach-Object { `
	$EachLine=@(); `
	$_.Properties | ForEach-Object { $EachLine += "$($_.Name)"; }; `
	Write-Output "$($($_.Name.PadRight(120)))  { $($EachLine -join ', ') }" | Out-File -Width 16384 -Append "${Logfile_txt}"; `
	Write-Output "$($_.Name)	$($EachLine -join '	')}" | Out-File -Width 16384 -Append "${Logfile_csv}"; `
} `
Notepad "${Logfile_txt}";
Notepad "${Logfile_csv}";
