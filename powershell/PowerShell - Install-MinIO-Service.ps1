...
If ((Test-Path "${WorkingDir}\mongod.exe") -Eq $False) { $ProtoBak=[System.Net.ServicePointManager]::SecurityProtocol; [System.Net.ServicePointManager]::SecurityProtocol=[System.Net.SecurityProtocolType]::Tls12; $ProgressPreference='SilentlyContinue'; Invoke-WebRequest -UseBasicParsing -Uri ("https://dl.min.io/server/minio/release/windows-amd64/minio.exe") -OutFile ("${Path_MainRuntime}"); [System.Net.ServicePointManager]::SecurityProtocol=$ProtoBak; };
If ((Test-Path "${WorkingDir}\${ServiceName}.exe") -Eq $False) { $ProtoBak=[System.Net.ServicePointManager]::SecurityProtocol; [System.Net.ServicePointManager]::SecurityProtocol=[System.Net.SecurityProtocolType]::Tls12; $ProgressPreference='SilentlyContinue'; Invoke-WebRequest -UseBasicParsing -Uri ("https://github.com/winsw/winsw/releases/download/v2.7.0/WinSW.NET4.exe") -OutFile ("${Path_ServiceInstaller}"); [System.Net.ServicePointManager]::SecurityProtocol=$ProtoBak; };
$ServiceArguments = @();
