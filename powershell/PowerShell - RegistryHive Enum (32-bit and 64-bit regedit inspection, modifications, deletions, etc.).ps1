...
$PropVal_64bit = $SubKey_64bit.GetValue($(If ("${PropertyName}" -Eq "(Default)") { "" } Else { "${PropertyName}" }));
$SubKey_64bit.Close();
...
$PropVal_32bit = $SubKey_32bit.GetValue($(If ("${PropertyName}" -Eq "(Default)") { "" } Else { "${PropertyName}" }));
$SubKey_32bit.Close();
