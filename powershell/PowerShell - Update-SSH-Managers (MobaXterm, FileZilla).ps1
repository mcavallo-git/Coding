...
$Pitchfork_RegexDeprecatedKeys = ('<Keyfile>([a-zA-Z]\:[\w\s\\\-_\.\,\(\)]+(?:.ssh|_SSH)[\w\s\\\-_\.\,\(\)]*(?:FIRSTNAME|LASTNAME)[\w\s\\\-_\.\,\(\)]*\.pem)</Keyfile>');
...
$Pitchfork_RegexDeprecatedUsers = ('<User>FIRSTNAME LASTNAME</User>');
