...
Function ESXi-Boot-Media() {
	...
	If (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]"Administrator")) {
		...
		If ($False) {
			...
			$LogFile = "${Home}\Desktop\ESXi.Get-EsxSoftwarePackage.${DepotOwner}.log"; Get-EsxSoftwarePackage | Sort-Object "Name" | Format-Table > "${LogFile}"; Notepad "${LogFile}";  <# Returns a list of SoftwarePackage (VIB) objects from connected depot(s) #> `
			$LogFile = "${Home}\Desktop\ESXi.Get-EsxSoftwarePackage.Verbose.${DepotOwner}.log"; Get-EsxSoftwarePackage | Sort-Object "Name" | Format-List > "${LogFile}"; Notepad "${LogFile}";  <# Returns a list of SoftwarePackage (VIB) objects from connected depot(s) #> `
