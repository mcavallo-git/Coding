	...
	If (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]"Administrator")) {
		...
		If ($CreateMedia -Eq $False) {
			...
			If ($PSBoundParameters.ContainsKey('AllDrivers')) {
				...
				Write-Host "`n`n";
				Write-Host "------------------------------------------------------------";
				Write-Host "Pulling updated list of ESXi software packages (.vib drivers) from depot @ `"$($Array_VibDepos[0])`"";
				...
				Write-Host "Pulling updated list of ESXi software packages (.vib drivers) from depot @ `"$($Array_VibDepos[1])`"";
