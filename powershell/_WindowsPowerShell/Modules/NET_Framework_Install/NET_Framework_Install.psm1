...
function NET_Framework_Install {
	...
	# Install (from Ninite) the following .NET Frameworks: [ 2019x64, 2019x86, 2017x64, 2017x86, 2015x64, 2015x86, 2010x64, 2010x86, 2008x64, 2008x86 ]
	$URL="https://ninite.com/.net4.8/ninite.exe"; $FN="${Env:Temp}\Install_.NET-Framework-4.8.exe"; Write-Host "Downloading/Installing `"${URL}`"..."; $(New-Object Net.WebClient).DownloadFile(([System.Net.HttpWebRequest]::Create("${URL}").GetResponse().ResponseUri.AbsoluteUri),$FN); Start-Process -Filepath ($FN) -ArgumentList (@("/q","/norestart"));
	$URL="https://ninite.com/.net4.0/ninite.exe"; $FN="${Env:Temp}\Install_.NET-Framework-4.0.exe"; Write-Host "Downloading/Installing `"${URL}`"..."; $(New-Object Net.WebClient).DownloadFile(([System.Net.HttpWebRequest]::Create("${URL}").GetResponse().ResponseUri.AbsoluteUri),$FN); Start-Process -Filepath ($FN) -ArgumentList (@("/q","/norestart"));
	$URL="https://ninite.com/.net3.5/ninite.exe"; $FN="${Env:Temp}\Install_.NET-Framework-3.5.exe"; Write-Host "Downloading/Installing `"${URL}`"..."; $(New-Object Net.WebClient).DownloadFile(([System.Net.HttpWebRequest]::Create("${URL}").GetResponse().ResponseUri.AbsoluteUri),$FN); Start-Process -Filepath ($FN) -ArgumentList (@("/q","/norestart"));
