function ProfileSync {
	...
	$Pro += (('Write-Host "')+($($MyInvocation.MyCommand.Name))+(' - Info: Detected PowerShell v$(($($PSVersionTable.PSVersion.Major))+($($PSVersionTable.PSVersion.Minor)/10))`n" -ForegroundColor Magenta;'));
	
	$Pro += (('Write-Host "')+($($MyInvocation.MyCommand.Name))+(' - Task: Loading personal and system profiles...`n" -ForegroundColor Magenta;'));
