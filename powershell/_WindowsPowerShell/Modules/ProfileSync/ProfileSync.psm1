function ProfileSync {
	...
	$Pro += (('$GithubOwner="')+(${GithubOwner})+('"; $GithubRepo="')+(${GithubRepo})+('";')+(' Write-Host "Starting: PowerShell Module Synchronization - `"https://github.com/${GithubOwner}/${GithubRepo}.git`"...`n" -ForegroundColor Cyan; If (Test-Path "${HOME}/${GithubRepo}") { Set-Location "${HOME}/${GithubRepo}"; git reset --hard "origin/master"; git pull; } Else { Set-Location "${HOME}"; git clone "https://github.com/${GithubOwner}/${GithubRepo}.git"; } . "${HOME}/${GithubRepo}/powershell/_WindowsPowerShell/Modules/ImportModules.ps1"; Write-Host "`nFinished: PowerShell Module Synchronized`n" -ForegroundColor Cyan;'));
