...
Function WakeOnLAN() {
	...
	If (!(($MAC -Like "*:*:*:*:*:*") -Or ($MAC -Like "*-*-*-*-*-*"))){
		...
		Write-Output "";
		Write-Error "Error:  Invalid syntax used for method `"$($MyInvocation.MyCommand.Name)`"";
		Write-Output "";
		Write-Error "Info:  Please call using syntax similar to:  [ `"$($MyInvocation.MyCommand.Name)`" `"${Example_MAC}`"; ]";
		Write-Output "";
		Start-Sleep 10;
