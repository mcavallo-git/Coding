...
If ($False) {
	$ToBeSigned=@(); ([System.IO.Directory]::EnumerateFiles("${Env:TEAMCITY_DATA_PATH}\system\artifacts\","*.msi","AllDirectories")) | Get-Item | Where-Object { (($_.LastWriteTime).AddMinutes(2) -lt (Get-Date)) } | Sort-Object -Property @{Expression={$_.LastWriteTime}; Ascending=$False} | ForEach-Object { $CheckSig=(Get-AuthenticodeSignature -FilePath ("$($_.FullName)")); If (($CheckSig.Status -NE "Valid") -Or ($CheckSig.TimeStamperCertificate -Eq $Null)) { $ToBeSigned += $_; Write-Output "`$ToBeSigned.Count :"; $ToBeSigned.Count; }; }; Write-Output "`n`nFinal Result:  `$ToBeSigned.Count :"; $ToBeSigned.Count; Write-Output "`n`n";
}
