...
## DO CODE SIGNING
If ($True) {
	<# $Artifacts_Msi=@(); (([System.IO.Directory]::EnumerateFiles("${Env:TEAMCITY_DATA_PATH}\system\artifacts\","*.msi","AllDirectories")) | ForEach-Object { $Artifacts_Msi += "$_"; }); Write-Output "`$Artifacts_Msi.Count :"; $Artifacts_Msi.Count; #>
...
If ($False) {
	...
	| ForEach-Object {
		...
		If (($CheckSig.Status -NE "Valid") -Or ($CheckSig.TimeStamperCertificate -Eq $Null)) {
			$ToBeSigned += $_; `
			Write-Output "`$ToBeSigned.Count :"; $ToBeSigned.Count; `
		}; `
	}; `
	Write-Output "`n`nFinal Result:  `$ToBeSigned.Count :"; $ToBeSigned.Count; Write-Output "`n`n";
}
